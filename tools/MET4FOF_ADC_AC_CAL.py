...
class Met4FOFADCCall:
    ...
    def CallFreq(self,Freq,Ampl,Channel,offset=0,phase=0,SamplingFreq=2002):
        ...
        self.FGen.ArmExtTrigSyncburst(Freq,Ampl,offset,phase,cycles,SamplingFreq,Samplingcycles)
        #fire Scope
        self.Scope.single()
        ...
        for i in range(self.bufferCount):
            self.DeltaTime[i]=self.buffer[i].unix_time-unix_time_start+(0-(unix_nsecs_start-self.buffer[i].unix_time_nsecs)*1e-9)
        ...
        retVal={'Freq':Freq,'Amplitude':amplTrans,'Phase':phaseFit,'TestAmplVPP.':Ampl}
