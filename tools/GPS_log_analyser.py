...
DF=pd.DataFrame(data=GPSLoggArray[:,1:],    # values
              index=GPSLoggArray[:,0],    # 1st column as index
              columns=['Secs','nSecs','uncer'])  # 1st row as the column names
...
if LANG=='DE':
    ...
    ax1.set_title('Abweichung der zeitgestempelten PTB PPS Signale nach Zeitstemplung durch DAQ Hardware')
ax1.grid(True)
ax1.legend()
fig1.show()
...
n, bins, patches = ax2.hist(DF['nSecDev'], bincount, density=1, facecolor='green', alpha=0.75)
...
if LANG=='EN':
    ...
    ax2.set_title(r'$\mathrm{Histogram\ of\ time deviation:}\ 2\sigma=%.3f ~ns,\ \sigma = %.3f$ ~ns n = $%i$' %(2*sigma, sigma,DF['nSecDev'].size))
if LANG=='DE':
    ax2.set_xlabel('Zeitabweichung $t_{(ist)}-t_{(soll)}$ in ns')
    ax2.set_ylabel('Häufigkeit')
    ax2.set_title(r'$\mathrm{Histogram~der~Zeitabweichungen:} \sigma = %.3f$ ns ,n = $%i$' %( sigma,DF['nSecDev'].size))
ax2.grid(True)
